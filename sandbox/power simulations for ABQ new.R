...
null2 <- glmmTMB(Abundance ~ (1|Location)+(1|Epi.wk)+(1|Epi.yr),
                 ...
                 ziformula=~1)
summary(null2)
simulationOutput <- simulateResiduals(fittedModel = null2)
...
ABQ5 <- ABQ2 %>%
  dplyr::select(Location, Species, Abundance, Date.2, Epi.wk, Epi.yr, geometry) %>%
  na.omit() %>%
  filter(st_coordinates(.)[,2]>34.96,
  st_coordinates(.)[,2]<35.193,
  st_coordinates(.)[,1]< -106.5,
  st_coordinates(.)[,1]> -106.75)
...
abq.sim <- function(...) {
  ...
  sim.glmm(design.data=dat,
           fixed.eff = list(intercept=log(median(data$Abundance)), 
                            Collection = log(c(UTRT = 1.0, 
                                               TRT  = 0.65)),
                            Block = log(c(One = 1.0,
                                          Two = 1.0,
