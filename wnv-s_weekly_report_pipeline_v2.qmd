...
bird_report = birds %>% 
  ...
  arrange(desc(test_code)) %>%
  mutate(wnv_result = if_else(test_code == 1, "positive", "negative")) %>%
  select(csu_id, year, week, wnv_result) 
...
# current_wk and pools produced earlier in this pipeline (calc_all chunk)
