...
# Uses wnvSurv::plot_n_trap; culex_update is wnv_s_clean()'d below (calc-zone-stats
# chunk), which carries the zone2 + trap_status columns the function requires.
culex_update = read.csv(fn_database_culex_update) %>% wnv_s_clean(verbose = F)
...
database_update = read.csv(fn_database_update) %>% wnv_s_clean(verbose = F)
