...
cq_data = merge_pcr_platemap(pcr_clean, platemap_clean) %>%
  ...
  #alternate test_code
inconclusive = read.csv(paste0(dir_input, "/inconclusive_decision.csv"))

cq_data = cq_data %>%
     mutate(test_code = if_else(csu_id %in% c("CSU23913"), 0, test_code))
...
bc_test_traps = database_new %>%
    filter(zone == "BC") %>%
    distinct(trap_id)

#keep only traps in routine active traps from surveillance
culex_new = all_spp %>%
  filter(trap_id %in% c(foco_trap$trap_id, bc_test_traps$trap_id)) %>%
