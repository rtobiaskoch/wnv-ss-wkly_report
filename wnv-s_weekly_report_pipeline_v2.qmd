...
update_gsheet(new = culex_new,
  ...
  # local merge only — skips GSheet push (offline / smoke test mode)
  culex_merged <- rquery::natural_join(culex_new, culex_database,
                   jointype = "FULL",
                   by = c("trap_id", "trap_date", "spp", "zone")) %>%
    dplyr::select(dplyr::all_of(names(culex_database)))
  if ("trap_date" %in% names(culex_merged)) {
    culex_merged <- culex_merged %>%
      dplyr::mutate(trap_date = lubridate::as_date(lubridate::as_datetime(trap_date))) %>%
      dplyr::arrange(dplyr::desc(trap_date))
  }
