...
culex_database = read.csv(fn_gdrive_culex_sheet) %>%
 # filter(trap_status != "missing") %>% #TO DO add to wnv_s_clean and remove
  ...
  # merge_trap_database()'s join on "key" (clean-update-all-spp-culex chunk
...
update_gsheet(new = culex_new,
  ...
  # local merge only — skips GSheet push (offline / smoke test mode).
  # Same merge_trap_database() the update=TRUE path uses (utils/fun_merge_trap_database.R).
  culex_merged <- merge_trap_database(new = culex_new,
                                      old = culex_database,
                                      by = "key",
                                      col_database = names(culex_database))
