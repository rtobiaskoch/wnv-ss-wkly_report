...
suppressMessages({
  ...
  pacman::p_load(
    argparse, #importing and exporting
    lubridate
  )
...
parser$add_argument(
  ...
  help = "week of report",
  type = "integer",
  default = week_hardcode
)
parser$add_argument(
  "--year",
  help = "year of the report",
  type = "integer",
  default = year_hardcode
)
parser$add_argument(
  "--n_hx_years",
  help = "number of prior seasons in the rolling historical baseline",
  type = "integer",
  default = 5
)
parser$add_argument(
  "--cp_threshold",
  help = "copy threshold for positive",
  type = "integer",
  default = 500
)
parser$add_argument(
  "--rn_threshold",
  help = "flourescent threshold for determining ct",
  type = "integer",
  default = 34000
)
parser$add_argument(
  "--vi_threshold",
  help = "vi threshold for plots",
  type = "integer",
  default = 0.75
)
parser$add_argument(
  "--update",
  help = "logical argument whether or not to update directory with data from gdrive",
  type = "logical",
  default = F
)
parser$add_argument(
  "--download",
  help = "logical argument whether or not to download data from gdrive",
  type = "logical",
  default = F
)
parser$add_argument(
  "--push",
  help = "logical argument whether or not to push directory with data to repository",
  type = "logical",
  default = F
)
...
saveRDS(
  all_params_list,
  file.path(dir_input, paste0(file_prefix, "config_weekly.RData"))
)
cat(
  "Config snapshot saved to:",
  file.path(dir_input, paste0(file_prefix, "config_weekly.RData")),
  "\n"
)
