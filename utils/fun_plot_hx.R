...
plot_hx = function(df, value, text, pallette = pal_mozzy) {
  ...
  # hx and current are drawn as two separate stacked-area layers on the SAME
  # zone panel (faceted by zone only) so they share one x/y axis and overlap
  # for direct comparison: hx (muted greys) behind, current (Pipiens/Tarsalis
  # colors) drawn on top with higher alpha.
...
plot_hx_line <- function(
  ...
  p <- ggplot(
    df,
    aes(x = week, y = value, group = interaction(!!color_var, type))
  ) +
    geom_hline(yintercept = 0) +
