check_data <- function(
  ...
  if ("csu_id" %in% names(df)) {
    if (nrow(get_dupes(df, csu_id)) > 0) {
      cat(
        "\n Alert! You have duplicate id's in ",
        df_name,
  ...
  if ("zone" %in% names(df)) {
    ...
    if (!any(str_detect(df$zone, zones))) {
      stop(paste0(
        "Column zone contains values that aren't ",
        zones,
        " in ",
        df_name
      ))
  ...
  if ("ct_threshold" %in% names(df)) {
    if (any(df$ct_threshold < rn_threshold)) {
      ...
      cat(
        "\nThe Ct Threshold (rn_threshold) is set to ",
        rn_threshold,
        ". Nice.\n"
      )
  ...
  if ("csu_id" %in% names(df) & "copies_WNV" %in% names(df)) {
    ...
    if (nrow(is_pcr) > 0) {
      ...
      if (
        any(pos$copies_WNV < copy_threshold | pos$copies_SLEV < copy_threshold)
      ) {
        warning(cat(
          "\n One of your positive extraction controls have < ",
          copy_threshold,
          " copies. \n"
        ))
      } else {
      ...
      if (
        any(neg$copies_WNV > copy_threshold | neg$copies_SLEV > copy_threshold)
      ) {
        warning(cat(
          "\n One of your negative extraction controls have < ",
          copy_threshold,
          " copies. \n"
        ))
      } else {
