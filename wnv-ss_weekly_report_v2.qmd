...
1.  Change Boulder week to epiweek
2.  check that all files have been uploaded to the googledrive
3.  check the quanstudio eds files on diomni pcr software on thermofisher and look to see if any inconclusives with a Ct are positive or negative. If any are negative add them to the amp_inconclusive_negative database sheet in the google drive
4.  download the entire wXX folder from googledrive into the input folder in this repository
...
Rscript config/config_weekly.R --year 2025 --week 37 --download T --update T --push T
...
#WNV DATABASE
gsheet_pull_prompt(fn_gdrive_database, sheet = "data", key = key_database_gsheet)
#SLEV DATABASE
gsheet_pull_prompt(fn_gdrive_database_slev, sheet = "data", key = key_database_gsheet_slev)
#CULEX SHEET
gsheet_pull_prompt(fn_gdrive_culex_sheet, sheet = "data", key = key_database_culex_sheet)
#RENAMING KEY
gsheet_pull_prompt(fn_key_rename, sheet = "Sheet1", key = key_rename_key)
#FOCO TRAP
gsheet_pull_prompt(fn_trap, sheet = "data", key = key_foco_trap)
#STANDARDS
gsheet_pull_prompt(fn_standards, sheet = "data", key = key_standards_gsheet)
#INCONCLUSIVE
gsheet_pull_prompt(fn_inconclusive, sheet = "data", key = key_inconclusive)
...
culex_database = read.csv(fn_gdrive_culex_sheet) %>%
 # filter(trap_status != "missing") %>% #TO DO add to wnv_s_clean and remove
  wnv_s_clean() %>%
  # Build the same composite key as culex_new (trap_id|spp|year|week) so
  # wnvSurv::update_join()'s join on "key" (clean-update-all-spp-culex chunk
  # below) has matching keys on both sides. trap_key_cols defined in
  # config/config_weekly.R.
  make_key(key_cols = trap_key_cols)
...
datasheet_input = read_list(dir_datasheet, "*")  %>%
  filter(!is.na(`Collection Site       (Trap ID)`)) #remove the csu-id at the end that will be for the following week
saveRDS(datasheet_input, fn_weekly_input_format_mid)

datasheet_clean = datasheet_input %>%
    key_rename(rename_df = df_key_rename,
             drop_extra = T) %>%
    wnv_s_clean(force_recompute = TRUE) # raw provider input: derive week/year from trap_date, ignore typed values
...
fn_path = list.files(path = dir_pcr, #
               pattern = "\\.xlsx?$", # only .xls/.xlsx; ignore the .eds and any other files in the pcr folder
               full.names = T,
               ignore.case = T)
...
# build_bird_report() from utils/fun_bird_report.R — pure extraction of bird
# shaping (cq_data is produced earlier in this pipeline). GSheet pushes and the
# plot output below remain here as pipeline side effects.

# Pull the full birds archive from GSheet so the plot spans all prior weeks of
# the current season (not just this week's PCR run). key_birds from config.
birds_hx <- googlesheets4::read_sheet(key_birds, sheet = "data")

bird_out    <- build_bird_report(cq_data, birds_hx = birds_hx,
                                  year_filter = year_filter, week_filter = week_filter)
birds               <- bird_out$birds
bird_report         <- bird_out$bird_report          # full archive, for plot_birds
bird_report_current <- bird_out$bird_report_current  # this week only, for the per-week sheet
...
if(update){
  ...
  if(bird_sheet %in% sheet_names(key_birds)) {
    sheet_delete(key_birds, sheet = bird_sheet)
  }

  safe_sheet_add <- possibly(sheet_add, otherwise = NULL)
  safe_sheet_add(ss = key_birds,
                 sheet = bird_sheet,
                 .after = 1)

  safe_sheet_write <- possibly(sheet_write, otherwise = NULL)
  safe_sheet_write(bird_report_current,
                   ss = key_birds,
                   sheet = bird_sheet)
}

# Bird plot output — WNV result counts by week (plot_birds from fun_bird_report.R)
p_birds <- plot_birds(bird_report, year = year_filter)
p_birds
...
p_pcr_wnv = plot_pcr(cq_data,
                 virus = "WNV",
                 copy_threshold = copy_threshold,
                 week_filter = week_filter)  # pattern_2_keep default now includes BOU + auto-prepends virus
...
if(update){
  ...
  # local merge only — skips GSheet push (offline / smoke test mode)
  wnv_merged <- rquery::natural_join(database_new, database,
                   jointype = "FULL",
                   by = c("csu_id", "trap_id", "trap_date", "spp", "zone")) %>%
  ...
  write.csv(wnv_merged, fn_database_update, row.names = FALSE)
}
...
if(update){
  ...
  # local merge only — skips GSheet push (offline / smoke test mode)
  slev_merged <- rquery::natural_join(database_slev_new, database_slev,
                   jointype = "FULL",
                   by = c("csu_id", "trap_id", "trap_date", "spp", "zone")) %>%
    dplyr::select(dplyr::all_of(names(database_slev_new))) %>%
    dplyr::mutate(trap_date = lubridate::as_date(lubridate::as_datetime(trap_date))) %>%
    dplyr::arrange(dplyr::desc(trap_date))
  write.csv(slev_merged, fn_database_slev_update, row.names = FALSE)
}
...
weekly_data_format = clean_4_weekly_input(fn_weekly_input_format_mid, database_new, dir_mid)
...
all_spp = all_spp0 %>%
  ...
  # spp0 (the raw species text) is now snapshotted by wnv_s_clean() itself, so
  # the hand-rolled mutate(spp0 = spp) that used to sit here is gone.
  wnv_s_clean(rm_col = c("trap_date"), force_recompute = TRUE) # raw provider input: derive week/year from trap_date, ignore typed values
...
culex_new = all_spp %>%
  ...
  # Build the same composite key the trap-history combiner uses
  # (wnvSurv::make_key -> "trap_id|spp|year|week"). This is the join key for the
  # database merge below: it excludes trap_date, so a new real row matches its
  # pre-seeded "no trap" stub (which has trap_date = NA) instead of duplicating it.
  make_key(key_cols = trap_key_cols)   # trap_key_cols defined in config/config_weekly.R
...
update_gsheet(new = culex_new,
  ...
  # local merge only — skips GSheet push (offline / smoke test mode).
  # Same wnvSurv::update_join() the update=TRUE path uses via update_gsheet().
  culex_merged <- wnvSurv::update_join(new = culex_new,
                                       old = culex_database,
                                       by = "key",
                                       col_keep = names(culex_database),
                                       arrange_desc = "trap_date")
  write.csv(culex_merged, fn_database_culex_update, row.names = FALSE)
}
...
# Uses plot_n_trap; culex_update is wnv_s_clean()'d here, which (with default
# all_cols) carries the zone2 + trap_status columns the function requires.
culex_update = read.csv(fn_database_culex_update) %>% wnv_s_clean(silence = T)

p_trap_status = plot_n_trap(culex_update, year_start = min(year_filter_hx), year_end = year_filter)
p_trap_status
...
# culex_update is already created (and wnv_s_clean()'d) in the trap-status-plot
# chunk above; reused here for calc_all().
...
```{r, plot-zone-stats}
hx2 = all_hx %>%
  filter(year %in% year_filter_hx) %>%   # same rolling baseline as `hx` (was hardcoded 2015:(year_filter-1))
  mutate(type = "hx")

ytd_hx_long = clean_long_hx(hx2, ytd, spp_keep = c("Pipiens", "Tarsalis", "All"))
write.csv(ytd_hx_long, file.path(dir_output, "ytd_hx_long.csv"))
...
ytd_hx_wk = clean_long_hx_wk(ytd, hx,
                          #  rm_zone = c("BC", fc_zones),
                            ...
                            spp_keep = c("Pipiens", "Tarsalis"))  # keep both spp for stacked plot
...
```{r, format-hx-B-tables, include=FALSE}
...
hx_abund = create_hx_report(df_all, 
                            zones = zone_lvls, 
                            prefix = "abund",
                            rm_col = c("abund_hx_LV", "abund_hx_BE", "abund_hx_BC"))
...
hx_pir = create_hx_report(df_all, 
                          ...
                          prefix = "pir", 
                          multiplier = 1000,
                          rm_col = c("pir_hx_LV", "pir_hx_BE", "pir_hx_BC"))

hx_vi = create_hx_report(df_all, 
                         zones = zone_lvls, 
                         prefix = "vi",
                         rm_col = c("vi_hx_LV", "vi_hx_BE", "vi_hx_BC"))
...
```{r format-A-tables, include=FALSE}
# build_tables() from utils/fun_build_tables.R — auto-sourced with all utils/*.R at pipeline top
# current_wk and pools are produced earlier in this pipeline (calc_all chunk)
...
# generate_report() from utils/fun_generate_report.R — auto-sourced with all
# utils/*.R at pipeline top. graph_sheet_layout / fn_graph_template /
# graph_week_cells come from config/config_weekly.R.
